...
export default {
  ...
  activate() {
    ...
    helpers.exec(atom.config.get('linter-kubectl.kubectlExecutablePath'), ['apply', '--help']).then(output => {
      if (!(/--validate=true/.exec(output))) {
        atom.notifications.addError(
          'The kubectl installed in your path is too old to support the kubectl apply --validate command.',
          {
  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        if (!(/apiVersion: v/.exec(activeEditor.getText())))
          return [];
        ...
        var args = ['apply', '--validate', '--dry-run'];

        // enable kubeconfig switching
        if (atom.config.get('linter-kubectl.kubeConfig') !== '')
          args = args.concat(['--kubeconfig', atom.config.get('linter-kubectl.kubectlContext')]);

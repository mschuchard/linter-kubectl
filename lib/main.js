      ...
      lint: async (textEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-kubectl.kubectlExecutablePath'), args, {stream: 'stderr', allowEmptyStderr: true}).then(output => {
          ...
          output.split(/\r?\n/).forEach((line) => {
            ...
            const connectMatches = /(\d+\.\d+\.\d+\.\d+:\d+): connect:/.exec(line)

            // first check for connection issue to cluster
            if (connectMatches != null) {
              toReturn.push({
                severity: 'info',
                excerpt: 'Kubectl is unable to connect to your configured cluster at ' + connectMatches[1],
                location: {
                  file: file,
                  position: [[0, 0], [0, 1]],
                },
